...
export function VoiceRecorder() {
  ...
  // Get AI audio volume for Orb reactivity (0-1 range)
  const getAiAudioVolume = useCallback(() => {
    if (!aiAnalyserRef.current) return 0
  ...
  // Map recording/AI state to Orb agent state
  const getOrbState = (): null | 'thinking' | 'listening' | 'talking' => {
    if (aiSpeaking) return 'talking'
    if (recordingState === 'processing') return 'thinking'
    // Note: we use LiveWaveform for 'listening', not Orb
    return null // idle
  }
